...
class KTextSurfaceWriter(object):
    ...
    text = property(
        lambda self: self._text, _setText, doc="""The text to be displayed"""
    )
    ...
    @classmethod
    def normalizeTextLength(
        cls,
        text_too_long,
        font,
        max_length,
        justify_chars=0,
        line_length_criteria=LL_CUT,
    ):
        ...
        while tooLong:
            ...
            if line_length_criteria.lower() == LL_CUT:
                ...
                while cls.wordTooLong(
                    word, font, max_length, justify_chars=justify_chars
                ):
                    word = word[:-1].strip()
                ...
                if cls.wordTooLong(word, font, max_length, justify_chars=justify_chars):
                    left_word = word
                    while cls.wordTooLong(
                        left_word, font, max_length, justify_chars=justify_chars
                    ):
                ...
                if cls.wordTooLong(word, font, max_length, justify_chars=justify_chars):
                    ...
                    if font.size(txt2)[0] <= max_length:
                        return cls.normalizeTextLength(
                            txt1, font, max_length, justify_chars=justify_chars
                        ) + [(" " * justify_chars) + word, txt2]
                    else:
                        return (
                            cls.normalizeTextLength(
                                txt1, font, max_length, justify_chars=justify_chars
                            )
                            + [(" " * justify_chars) + word]
                            + cls.normalizeTextLength(
                                txt2, font, max_length, justify_chars=justify_chars
                            )
                        )
            else:
                raise ValueError(
                    "Invalid line_length_criteria value: %s" % line_length_criteria
                )
        ...
        if font.size(txt2)[0] <= max_length:
            ...
            return [txt1] + cls.normalizeTextLength(
                txt2, font, max_length, justify_chars=justify_chars
            )
    ...
    def _getPreparedText(self):
        ...
        for line in text.split("\n"):
            ...
            if lw > rw:
                newtextlines = self.normalizeTextLength(
                    line,
                    self.font,
                    rw,
